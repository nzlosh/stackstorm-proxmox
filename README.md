...
| Parameter | Type | Required | Secret | Description |
|---|---|---|---|---|
| `limit` | integer | False | default | _The maximum amount of lines that should be printed._ |
| `node` | string | True | default | _The cluster node name._ |
| `start` | integer | False | default | _The line number to start printing at._ |
| `upid` | string | True | default | _The task's unique ID._ |
| `profile_name` | string | False | default | _The profile to use to run the action._ |
...
| `notes_template` | string | False | default | _Template string for generating notes for the backup(s). It can contain variables which will be replaced by their values. Currently supported are {{cluster}}, {{guestname}}, {{node}}, and {{vmid}}, but more might be added in the future._ |
...
| `hotplug` | string | False | default | _Selectively enable hotplug features. This is a comma separated list of hotplug features: 'network', 'disk', 'cpu', 'memory' and 'usb'. Use '0' to disable hotplug completely. Using '1' as value is an alias for the default `network,disk,usb`._ |
